...
class GPT_Compressive(tf.keras.Model):
    ...
    p_keep=0.9, p_reg=1.0, var_type="norm_add", **kwargs):
        ...
        # GPT Variables. #
        pos_seq_shape  = [
            self.n_layers, self.n_tot, self.hidden_size]
    ...
    def transformer_decode(
        ...
        if training:
            ...
            p_keep = 1.0
        input_step = tf.shape(dec_inputs)[1]
        ...
        for m in range(self.n_layers):
            ...
            conc_input = tf.concat([
                mem_inputs[m], layer_input], axis=1)
            if self.var_type == "norm_add":
                layer_in = tf.add(
                    tmp_pos, layer_normalisation(
                        conc_input, tmp_i_bias, tmp_i_scale))
            elif self.var_type == "add_norm":
                layer_in = layer_normalisation(tf.add(
                    tmp_pos, conc_input), tmp_i_bias, tmp_i_scale)
            ...
            layer_input = x_self_out[:, -input_step:, :]
            list_outputs.append(tf.expand_dims(
                x_self_out[:, -input_step:, :], axis=0))
        ...
        if self.var_type == "norm_add":
            dec_outputs = tf.add(
                dec_inputs, layer_normalisation(
                    layer_outputs[-1], tmp_o_bias, tmp_o_scale))
        elif self.var_type == "add_norm":
            dec_res_out = tf.add(dec_inputs, layer_outputs[-1])
    ...
    def call(self, x_input, training=False):
        ...
        for n_block in range(self.n_blocks):
            ...
            tot_step = self.nm + self.ncm + seq_step
            
            # Concatenate the memory inputs. #
            x_mem_inputs = tf.concat([
            ...
            tmp_out_tuple = self.transformer_decode(
                tot_step, x_dec_embed, x_mem_inputs, training=training)
    ...
    def compute_loss(self, x_input, x_output):
        ...
        for n_block in range(self.n_blocks):
            ...
            # Transformer Decoder for current block. #
            tmp_out_tuple = self.transformer_decode(
                tot_step, x_dec_embed, x_mem_inputs, training=True)
    ...
    def infer(self, x_infer):
        ...
        for step in range(self.seq_length):
            ...
            total_step = self.nm + self.ncm + local_step
